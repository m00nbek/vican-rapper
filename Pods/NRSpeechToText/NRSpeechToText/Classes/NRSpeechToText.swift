...
open class NRSpeechToText {
	...
	public static let shared = NRSpeechToText()
    //Private variables
   ...
   open func authorizePermission(completion: @escaping (Bool) -> Void) {
    ...
    public func startRecording(handler: @escaping (_ result: String?, _ isFinal: Bool, _ error: Error?) -> Void) {
        ...
        recognitionTask = speechRecognizer.recognitionTask(with: recognitionRequest, resultHandler: { (result, error) in  //7
            ...
            //Recognizes speech from the audio source associated with the specified request, using the specified completion handler to handle the results.The block can also be called repeatedly with partial results when shouldReportPartialResults is true.
